...
import { ERROR_INVALID_TOKEN, ERROR_NOT_FOUND } from './constants';
...
function mapValidationError(error: unknown): AuthError {
  ...
  return new AuthError(500, 'An unexpected error occurred during token validation.');
}
...
export async function verifyRequestToken(
  ...
  const token = spaceIndex === -1 ? '' : authorization.slice(spaceIndex + 1).trim();
