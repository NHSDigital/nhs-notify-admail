...
export default function AIFeedback({ feedback, isLoading }) {
  ...
  const returnContent = () => {
    ...
    if (
      ...
      feedbackObj.includes(
        "not an acceptable input prompt and has been rejected.",
      )
    ) {
      return (
        <p className="no-feedback">
          AI Guardrails have rejected this letter, please edit and resubmit
        </p>
      );
    }
    if (feedbackObj === null) {
        ...
        <div className="assessment-container">
          ...
          <div className="assessment-content">
            ...
            <span
              className={
                getRatingClass(
                  cleanedFeedback.rating || cleanedFeedback.Rating,
                ) || "rating-default"
              }
            >
            ...
            <ReactMarkdown>
              {cleanedFeedback.reason || cleanedFeedback.Reason || ""}
            </ReactMarkdown>
            <br />
            <strong style={{ fontSize: "19px" }}>Advice:</strong>
            <ReactMarkdown>
              {cleanedFeedback.advice || cleanedFeedback.Advice || ""}
            </ReactMarkdown>
          </div>
        </div>
      );
  ...
  return (
    <div className="ai-feedback">
      <div className="feedback-box">{returnContent()}</div>
    </div>
  );
}
