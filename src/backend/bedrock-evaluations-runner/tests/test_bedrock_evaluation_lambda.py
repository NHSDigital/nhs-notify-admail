...
def test_lambda_handler_success():
    ...
    mock_evaluator.run_evaluation_job.return_value = {"jobName": "job", "jobArn": "arn", "consoleUrl": "url"}
    with patch("bedrock_evaluation_lambda.BedrockEvaluator", return_value=mock_evaluator):
        event = {}
        context = {}
...
def test_lambda_handler_internal_error():
    with patch("bedrock_evaluation_lambda.BedrockEvaluator", side_effect=Exception("fail")):
        event = {}
        context = {}
        response = lambda_mod.lambda_handler(event, context)
